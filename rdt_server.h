...
void handle_client_adv(int socketfd, fd_set *master) {
	...
	if (nbytes <= 0) {
		...
		if (socketfd == client1_sockfd) {
			if (msgrecvd.type == DATA_F) {
				printf("SERVER: received frame with packet type %d\n", msgrecvd.pkt.msgType);
			}
...
void *receiver1(void *param) {
	...
	char fileRecvName[FILENAMESIZE];				// File name of file currently being received
	...
	while(1) {
		...
		//deliver_data(1, client1_sockfd); //change to get data from packet buffer
		
		if ( pktRecvd == NULL ) { continue; }

		// printf("SERVER: received packet, message type %d\n", pktRecvd->msgType);
		// printf("INIT FP NULL? %d\n", fp==NULL);
		// printf("INIT isReceiving? %d\n", isReceiving);
		// printf("INIT FfileRecvName? %s\n", fileRecvName);
		// printf("INIT counter? %d\n", counter);
		if ( (*pktRecvd).msgType == CHAT_M || (*pktRecvd).msgType == CONFIRM_M ) {
			if ( (*pktRecvd).msgType == CHAT_M ) { printf("Received %s\n", CHAT); }
			else if ( (*pktRecvd).msgType == CONFIRM_M ) { printf("Received %s\n", CONFIRM); }
			...
			if ( isReceiving == false ) {
				...
				if ( access((*pktRecvd).filename, F_OK) != -1 ) {
					char prepend[FILENAMESIZE];
					sprintf( prepend, "%i", counter );
					strcpy( fileRecvName, prepend );
					...
					strcpy( fileRecvName, (*pktRecvd).filename);
				}
				...
				if (fp!=NULL) {
					strcpy(GFILENAME, fileRecvName);
					printf("SETTING GFILENAME: %s\n", GFILENAME);
				}

				fwrite( (*pktRecvd).data, 1, (*pktRecvd).filebytesize, fp );
			...
			printf("SERVER: Got a file transfer end\n");
			isReceiving = false;
			printf( "File saved.\n" );

			// printf("GFILENAME: %s\n", GFILENAME);
			// sendFilePackets(GFILENAME);
		}
...
void createMessage(Packet * pktRecvd) {
	...
	printf("SERVER: createMessage called\n");
	...
	if ( (*pktRecvd).msgType == CHAT_M || (*pktRecvd).msgType == CONFIRM_M ) {
		...
		if (is_gbn == 1) {
			msg.sockfd = client1_sockfd;

			BufferEntry * pbe = malloc(sizeof(BufferEntry));
			pbe->pkt = msg;

			push_pkt_snd_buffer(1, client1_sockfd, pbe);
		}
	}
	/*else if ( strcmp(command, QUIT ) == 0) {
		...
		if (is_gbn == 1) {
			...
			push_pkt_snd_buffer(0, client1_sockfd, pbe);
		}
	}*/
	else if ( (*pktRecvd).msgType == MESSAGE_M ) {
		printf("SERVER: Got a message command\n");
		msg.msgType = MESSAGE_M;
		strncpy( msg.alias, "SERVER", strlen("SERVER") );
		strncpy( msg.data, (*pktRecvd).data, strlen((*pktRecvd).data) );
		...
		if (is_gbn == 1) {
			...
			printf("SERVER: Pushing packet into send packet buffer\n");
			push_pkt_snd_buffer(1, client1_sockfd, pbe);
		}
	}
...
void write_stats_to_file(){
	...
	fp = fopen("server_stats.dat", "w");
	fprintf(fp, "SERVER STATS\n");
	fprintf(fp, "==========================================\n");
	fprintf(fp, "data_frames_sent = %d\n", data_frames_sent);
	fprintf(fp, "dup_frames_recvd = %d\n", dup_frames_recvd);
	fprintf(fp, "data_frames_retransmits = %d\n", data_frames_retransmits);
	fprintf(fp, "frame_acks_sent = %d\n", frame_acks_sent);
	fprintf(fp, "frame_acks_recvd = %d\n", frame_acks_recvd);
	fprintf(fp, "total_data_sent = %d\n", total_data_sent);
	fclose(fp);
}
