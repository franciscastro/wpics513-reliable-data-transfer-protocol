...
void createMessage(const char* command, const char* message) {
	...
	if ( strcmp(command, CONNECT) == 0 ) {
		...
		msg.msgType = CHAT_M;
		strcpy( msg.alias, message);
		strcpy( alias, msg.alias);		// Set this client's global alias

		if (is_gbn == 1) {
			printf("Pushing chat request packet into send packet buffer\n");
	...
	/*else if ( strcmp(command, QUIT ) == 0) {
		...
		strcpy( msg.alias, alias);		// Set this client's global alias

		printf( "[ %s ]: %s\n", msg.alias, msg.data );
		if (is_gbn == 1) {
			printf("Pushing message packet into send packet buffer\n");
			msg.sockfd = client_sockfd;
		...
		if (is_gbn == 1) {
			printf("Pushing packet into send packet buffer\n");
			msg.sockfd = client_sockfd;
...
void sendFilePackets(const char* filename) {
	...
	while( !feof(fp) ){
		...
		// datalinkSend( client_sockfd, msg );
		if (is_gbn == 1) {
			printf("Pushing file packet into send packet buffer\n");
			msg.sockfd = client_sockfd;
...
void help() {
	...
	printf("\t%-10s request to a chat with server\n", CHAT);
...
void *receiver(void *param) {
	...
	while(1) {	
		...
		Packet * pktRecvd = extract_data(0, client_sockfd);
...
void write_stats_to_file(){
	...
	fp = fopen("client_stats.dat", "w");
	fprintf(fp, "CLIENT STATS\n");
	fprintf(fp, "==========================================\n");
